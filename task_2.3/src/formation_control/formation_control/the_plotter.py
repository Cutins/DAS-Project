...
class Plot(Node):
    def __init__(self):
        ...
        self.pos_target = np.zeros((self.n_agents, 3))
        ...
        if all_received: 
            ...
            if all_synch:       
                ...
                if self.kk > 5:
                    ...
                    if np.all(self.formation[self.kk]):
                        ...
                        if self.start_moving == 0: # Detect rising edge, first time we reach formation 
                            ...
                            # Calcolate the Target position for all the formation
                            for agent in range(self.n_agents):
                                self.pos_target[agent] = self.pos[self.kk, agent] + self.target

                            # Send starting moving and all the target position
                            msg = MsgFloat()
                ...
                if self.kk == self.max_iters:
                    ...
                    plt.figure('Formation potential -  check')
                    for agent in range(self.n_agents):
                        plt.plot(range(self.max_iters), self.formation_potential[:,agent], ':', label=f'Agent {agent}') 
                    plt.plot(range(self.max_iters), np.mean(self.formation_potential, axis=-1), label=f'Mean over all the agents', linewidth = 2)
