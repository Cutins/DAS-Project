...
images_test = np.array([df_train_balanced['image'][agent::N_AGENTS].tolist() for agent in range(N_AGENTS)])
labels_test = np.array([df_train_balanced['label'][agent::N_AGENTS].tolist() for agent in range(N_AGENTS)])
...
#     for batch_el in range(BATCH_SIZE):
#         idx = ((N_BATCH-1)*BATCH_SIZE) + batch_el
#         print(f"[Agent {agents}] Label for Image {idx} was {labels[agent,idx]} but is classified as {(forward_pass(images_test[agent,idx])[-1,0], uu[agent]):.4f}")
#     print()
...
for agent in range(N_AGENTS):
    for img in range(SAMPLES_PER_AGENT):
        output = forward_pass(images_test[agent,img], uu[agent])
        success, error = accuracy(output[-1, 0], labels_test[agent,img])
