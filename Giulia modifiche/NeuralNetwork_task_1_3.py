...
print('Row Stochasticity {} \n'.format(np.sum(WW,axis=1)))
...
prediction = np.zeros((N_AGENTS, N_BATCH,BATCH_SIZE))
    ...
    for batch_num in range(N_BATCH):
        for agent in range(N_AGENTS):
            ...
            for batch_el in range(BATCH_SIZE):
                ...
                prediction[agent, batch_num, batch_el] = pred[0]
...
for agent in range(N_AGENTS):
    for img in range(SAMPLES_PER_AGENT):
        success, error = accuracy((prediction[agent].flatten())[img],labels[agent,img])
        successes[agent] += success
        errors[agent] += error

    percentage_of_success[agent] = (successes[0]/(SAMPLES_PER_AGENT))*100
    print('\nAGENT: ', agent)
    print("Correctly classified point: ", successes[0])
    print("Wrong classified point: ", errors[0])
...
plt.plot(range(EPOCHS),np.sum(J, axis=-1)/N_AGENTS, label='Total Cost Evolution')
for agent in range(N_AGENTS):
     plt.plot(range(EPOCHS), J[:, agent])
plt.xlabel(r'Epochs')
plt.ylabel(r'J')
plt.title('J')
plt.grid()
...
plt.semilogy(range(EPOCHS), np.sum(NormGradientJ, axis=-1), label='Total Gradient Evolution')
plt.xlabel(r'Epochs')
