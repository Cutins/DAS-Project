...
prediction = np.zeros((N_BATCH,BATCH_SIZE))
...
for epoch in range(EPOCHS):
    ...
    for batch_num in range(N_BATCH):
        ...
        for batch_el in range(BATCH_SIZE):
            ...
            pred = xx[batch_el, -1, :]
            prediction[batch_num,batch_el] = pred[0]
...
for img in range(SAMPLES):
    success, error = accuracy((prediction.flatten())[img],labels[img])
    successes += success
    errors += error
