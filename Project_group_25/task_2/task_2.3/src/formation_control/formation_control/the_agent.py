...
def linear_trajectory (amplitude):
    x = 0
    y = amplitude
    z = 0
...
def trajectory_to_target (target, actual, gain = 1):
    gradient = np.zeros(3)
    for idx in range(3):
            ...
            if all_received:
                ...
                if all_synch:
                    ...
                    if self.start_moving and self.type: # Leader
                        ...
                        if self.type_motion == 0: # circular
                            discrete_time = self.kk - self.k_start_moving
                            delta_pos = delta_pos + (circle_trajectory(self, discrete_time, self.amplitude))

                        elif self.type_motion == 1: # target_position
                            delta_pos = delta_pos + trajectory_to_target(self, self.pos_target, self.pos)

                        elif self.type_motion == 2: # linear
                            delta_pos = delta_pos + linear_trajectory(self.amplitude)
